...
function uploadtodrive($filename,$filetype,$fullpath)
{
  require_once $_SERVER["DOCUMENT_ROOT"].'/google/google-api-php-client/src/Google/Client.php';
  require_once $_SERVER["DOCUMENT_ROOT"].'/google/google-api-php-client/src/Google/Service/Oauth2.php';
  require_once $_SERVER["DOCUMENT_ROOT"].'/google/google-api-php-client/src/Google/Service/Drive.php';
  ...
  $file_tmp_name = $fullpath;

  // Get the client Google credentials
  $credentials = $_SESSION['access_token'];
  // Get your app info from JSON downloaded from google dev console
  $json = json_decode(file_get_contents($_SERVER["DOCUMENT_ROOT"]."/storage/secret/credentials.json"), true);
  ...
  */
  ...
  return $driveInfo["alternateLink"];
}
